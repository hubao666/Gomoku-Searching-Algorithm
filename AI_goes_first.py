...
def play_the_chess():
    ...
    '''
    first_piece_X = random.randint(6, 10)
    first_piece_Y = random.randint(6, 10)
    first_piece = Circle(Point(BOX_WIDTH * first_piece_X, BOX_WIDTH * first_piece_Y), CHESS_RADIUS)
    ...
    while not is_gameOver:
        ...
        if turn % 2 == 0:
            ...
            if first_piece:
                first_piece.setFill('white')
                first_piece.draw(window)
                first_piece = None
                player2_list.append((first_piece_X, first_piece_Y))
                all_list.append((first_piece_X, first_piece_Y))
                turn += 1
                continue
            pos2 = AI_algo()
            pos2_X = pos2[0]
